...
def split_content_haystack():
    try:
        ...
        for i in range(len(df_test)):
            ...
            # print(fragments_list[:num_rows_to_add])

            # if i == 0:
            #     new_df['split'] = fragments_list[:num_rows_to_add]
            #     new_df['split_id'] = split_id_list[:num_rows_to_add]
            #     new_df['tokens'] = tokens_list[:num_rows_to_add]
            # else:
            #     # new_df.concat([new_df, {'split': fragments_list[i], 'split_id': split_id_list[i], 'tokens': tokens_list[i]}])
            #     new_df.loc[len(new_df)] = {'split': fragments_list[i], 'split_id': split_id_list[i], 'tokens': tokens_list[i]}
